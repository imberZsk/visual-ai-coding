...
const IPC = {
  ...
  GET_UNIFIED_MCP: 'get-unified-mcp',
  SAVE_UNIFIED_MCP: 'save-unified-mcp',
  LIST_UNIFIED_SKILLS: 'list-unified-skills',
  SYNC_UNIFIED: 'sync-unified',
}
...
const api = {
  ...
  getOfficialSettingsSources: () =>
    ipcRenderer.invoke(IPC.GET_OFFICIAL_SETTINGS_SOURCES),
  // updateOfficialSettingsSources 同步官方设置来源。
  updateOfficialSettingsSources: () =>
    ipcRenderer.invoke(IPC.UPDATE_OFFICIAL_SETTINGS_SOURCES),
  // readConfigFile 读取单个配置文件。
  readConfigFile: (payload) =>
    ipcRenderer.invoke(IPC.READ_CONFIG_FILE, payload),
  // saveConfigFile 保存单个配置文件。
  saveConfigFile: (payload) =>
    ipcRenderer.invoke(IPC.SAVE_CONFIG_FILE, payload),
  // listDir 列出目录条目。
  ...
  listClaudeOutputStyles: (claudeHome) =>
    ipcRenderer.invoke(IPC.LIST_CLAUDE_OUTPUT_STYLES, claudeHome),
  // createClaudeOutputStyle 创建 Claude output style 文件。
  createClaudeOutputStyle: (payload) =>
    ipcRenderer.invoke(IPC.CREATE_CLAUDE_OUTPUT_STYLE, payload),
  // listClaudePlugins 读取 Claude 插件列表。
  listClaudePlugins: (claudeHome) =>
    ipcRenderer.invoke(IPC.LIST_CLAUDE_PLUGINS, claudeHome),
  ...
  checkCodexPluginUpdates: (codexHome) =>
    ipcRenderer.invoke(IPC.CHECK_CODEX_PLUGIN_UPDATES, codexHome),
  ...
  checkToolLatestVersion: (toolId) =>
    ipcRenderer.invoke(IPC.CHECK_TOOL_LATEST_VERSION, toolId),
  ...
  // getUnifiedMcp 读取统一 MCP 配置源。
  getUnifiedMcp: () => ipcRenderer.invoke(IPC.GET_UNIFIED_MCP),
  // saveUnifiedMcp 保存统一 MCP 配置源。
  saveUnifiedMcp: (servers) =>
    ipcRenderer.invoke(IPC.SAVE_UNIFIED_MCP, servers),
  // listUnifiedSkills 列出统一 Skills 源目录下的技能。
  listUnifiedSkills: () => ipcRenderer.invoke(IPC.LIST_UNIFIED_SKILLS),
  // syncUnified 把统一配置一键同步到 Claude 与 Codex。
  syncUnified: (options) => ipcRenderer.invoke(IPC.SYNC_UNIFIED, options),
}
