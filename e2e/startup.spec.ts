...
async function captureStartupSplash(
  ...
  await app.evaluate(async ({ BrowserWindow, session }) => {
    ...
    const previewSession = session.fromPartition(`vac-startup-${Date.now()}`)
    ...
    globalThis.__visualAiCodingStartupPreview = previewWindow
...
electronTest(
  '冷启动展示项目图标和统一 loading',
  async (_page, app, testInfo) => {
