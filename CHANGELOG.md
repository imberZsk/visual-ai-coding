...
- 新增独立 Marketplace 入口，用于管理市场来源与本地索引，并与已安装插件管理分开展示。
- Electron 冷启动阶段新增跟随明暗主题的应用图标与加载反馈，并补充启动链路和关键页面的无头 E2E 验收。

### 优化

- 统一侧边栏、概览、设置、额度与统一配置页的紧凑布局、Ant Design 语义色和组件级样式边界，同时完善 Electron 无标题栏窗口的拖动与交互区域。
- 完善工具偏好、插件来源与渲染层 API 的持久化和空值处理，并补充对应核心、IPC 与 UI 测试。
...
- Release 工作流改用安装包与自动更新文件显式白名单，避免上传 `builder-debug.yml` 等构建调试文件。
