...
// 该测试套件用于验证 Tauri beforeDevCommand 遇到端口冲突时的决策行为。
describe("resolveDevServerAction", () => {
  // 该回调用于验证端口已有可访问 dev server 时直接复用，避免 Tauri dev 因重复启动 Vite 失败。
  ...
  it("selects the next free port when the default port is occupied", async () => {
    ...
    expect(selectedPort).toBe(1421);
    expect(checkedPorts).toEqual([1420, 1421]);
  });

  // 该回调用于验证启动 Vite 时会显式传入选中的端口并保持 strictPort。
  it("builds Vite args with the selected host and port", () => {
    // args 存储传给 Vite CLI 的最终参数。
    const args = buildViteArgs(["--clearScreen", "false"], "127.0.0.1", 1421);
  ...
  // 该回调用于验证端口被非 HTTP 服务占用时仍返回 blocked，避免 Tauri 连到错误服务。
