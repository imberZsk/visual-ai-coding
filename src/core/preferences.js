...
function normalizePreferences(value) {
  // activeAiTool 存储校验后的当前 AI 工具，旧版本或非法值统一回退到 Codex。
  const activeAiTool = value?.active_ai_tool === 'claude' ? 'claude' : 'codex'
  return {
    ...defaultPreferences(),
    ...(value && typeof value === 'object' ? value : {}),
    active_ai_tool: activeAiTool,
    hidden_visual_config_fields:
      value?.hidden_visual_config_fields &&
      typeof value.hidden_visual_config_fields === 'object'
        ? value.hidden_visual_config_fields
        : {},
  }
}
