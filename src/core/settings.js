...
export function validateContent(content, format) {
  ...
  if (format === 'toml') {
    try {
      ...
      throw new Error(`TOML 格式错误: ${error.message}`, { cause: error })
    }
  }
}
...
export async function listDir(dirPath) {
  ...
  const entries = statResults.filter(Boolean).sort((left, right) => {
...
async function readCustomOutputStyle(filePath) {
  ...
  const description =
    parseFrontMatterValue(content, 'description') || `自定义输出风格：${name}`
