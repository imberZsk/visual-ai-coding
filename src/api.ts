...
// 读取统一 MCP 配置源（中立格式，供两端复用）
export function getUnifiedMcp(): Promise<UnifiedMcpResult> {
  return getElectronApi().getUnifiedMcp()
}

// 保存统一 MCP 配置源
// servers 参数存储用户编辑后的 MCP server 列表。
export function saveUnifiedMcp(
  servers: UnifiedMcpServer[]
): Promise<UnifiedMcpResult> {
  return getElectronApi().saveUnifiedMcp(servers)
}

// 列出统一 Skills 源目录下的技能名称
export function listUnifiedSkills(): Promise<UnifiedSkillsResult> {
  return getElectronApi().listUnifiedSkills()
}

// 把统一配置一键同步到 Claude 与 Codex
// options 参数存储可选的 claudeHome / codexHome 覆盖路径。
export function syncUnified(options?: {
  claudeHome?: string
  codexHome?: string
}): Promise<UnifiedSyncResult> {
  return getElectronApi().syncUnified(options)
}
