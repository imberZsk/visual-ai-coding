...
vi.mock("./Dashboard", () => ({
  DashboardContent: () => <div>工具状态概览</div>,
}));

vi.mock("../api", () => ({
  getOfficialSettingsSources: (...args: unknown[]) => getOfficialSettingsSourcesMock(...args),
  updateOfficialSettingsSources: (...args: unknown[]) => updateOfficialSettingsSourcesMock(...args),
}));
...
function createOfficialSourcesResult(cachedAt: string): OfficialSettingsSyncResult {
  return {
    sources: [
      {
        id: "claude-settings",
...
describe("SettingsContent", () => {
  beforeEach(() => {
    ...
    getOfficialSettingsSourcesMock.mockResolvedValue(createOfficialSourcesResult(""));
  ...
  it("shows official settings sources and refreshes official metadata", async () => {
    ...
    expect(screen.getByText("https://docs.anthropic.com/en/docs/claude-code/settings")).toBeInTheDocument();
    ...
    const claudeSourceCard = screen.getByTestId("official-source-claude-settings");
    expect(within(claudeSourceCard).getByText("官方字段 2 / 已覆盖 1 / 未覆盖 1")).toBeInTheDocument();
    expect(within(claudeSourceCard).getByText("futureClaudeFlag")).toBeInTheDocument();
    expect(within(claudeSourceCard).getByText("2026-07-01T01:02:03Z")).toBeInTheDocument();
  });
  ...
  it("uses unified loading while refreshing official settings", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("button", { name: "更新官方设置" })).not.toBeDisabled();
    });
  });
