...
import { Alert, Empty, Input, Segmented, Select, Spin } from 'antd'
...
// AI_TOOL_OPTIONS 存储设置页默认 AI 工具选项。
...
const SCHEMA_FIELD_PATHS: Record<string, Set<string>> = {
  ...
  'codex-config': new Set(
    CODEX_CONFIG_SCHEMA.groups.flatMap((group) =>
      group.fields.map((field) => field.path)
    )
  ),
}
...
function summarizeOfficialSource(
  source: OfficialSettingsSource
...
export function SettingsContent() {
  ...
  const [officialSettings, setOfficialSettings] =
    useState<OfficialSettingsSyncResult | null>(null)
  ...
  return (
    <div className="settings-content">
      ...
      {/* 默认 AI 工具 */}
      <Card className="settings-section">
        <SectionTitle>AI 工具</SectionTitle>
        <p className="settings-section__description">
          侧栏一次只展示一个工具的配置入口，可随时通过侧栏选择器切换。
        </p>
        <label htmlFor="settings-ai-tool" className="settings-field-label">
          默认显示工具
        </label>
        <Select
          id="settings-ai-tool"
          ...
          value={prefs?.active_ai_tool || 'codex'}
          onChange={(activeAiTool) =>
            updatePrefs({ active_ai_tool: activeAiTool })
          }
        />
      </Card>
      ...
      <Card className="settings-section">
        ...
        <p className="settings-section__description">
          指定编辑器命令和两个 AI 工具的本地配置目录。
        </p>
        <div className="settings-field-list">
          {/* VSCode CLI 路径 */}
          <div>
            <label
              htmlFor="settings-vscode-path"
      ...
      <Card className="settings-section">
        <div className="settings-section__header">
          ...
          <Button
            onClick={handleUpdateOfficialSettings}
            variant="default"
        ...
        <div className="settings-source-list">
          {(officialSettings?.sources ?? []).map((source) => {
            ...
            const hiddenMissingCount = Math.max(
              summary.missingFields.length - missingPreview.length,
              0
            )
            ...
            return (
              ...
              >
                ...
                <div className="mt-2 text-xs text-text-muted">
                  官方字段 {source.fields.length} / 已覆盖{' '}
                  {summary.coveredCount} / 未覆盖 {summary.missingFields.length}
                </div>
          ...
          {!officialLoading &&
            officialSettings &&
            officialSettings.sources.length === 0 && (
              <Empty
                image={Empty.PRESENTED_IMAGE_SIMPLE}
                description="暂无官方设置来源"
              />
            )}
          ...
          {officialSettings?.diagnostics && (
            <Alert
              message={officialSettings.diagnostics}
              showIcon
              type="warning"
            />
          )}
          {officialError && (
            <Alert message={officialError} showIcon type="error" />
          )}
        </div>
      </Card>
    </div>
