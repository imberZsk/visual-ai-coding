...
function getFieldToggle(title: string): HTMLElement {
  ...
  const toggleNode = titleNode.closest(
    '.ant-collapse-header'
  ) as HTMLElement | null
...
function getFieldShell(title: string): HTMLElement {
  ...
  const shellNode = toggleNode.closest(
    '.visual-config-field'
  ) as HTMLElement | null
...
function getAntSelectText(title: string): string {
  ...
  const selectionNode =
    fieldShell.querySelector(
      '.ant-select-content-value, .ant-select-content, .ant-select-selection-item'
    ) ??
...
async function chooseAntSelectOption(optionText: string) {
  ...
  await waitFor(() => {
    ...
    optionNode =
      optionNodes.find((node) => node.textContent?.includes(optionText)) ?? null
...
describe('VisualConfigEditor', () => {
  beforeEach(() => {
    ...
    updatePrefsMock.mockImplementation(
      async (patch: Partial<typeof prefsMock>) => {
    ...
    invokeMock.mockResolvedValue({
      ...
      content: JSON.stringify(
        { model: 'opus', customFutureFlag: true },
        null,
        2
      ),
      exists: true,
  ...
  it('renders known fields and unknown advanced fields', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /显示高级字段/ })
    ).toBeInTheDocument()
  ...
  it('renders visual config controls with Ant Design building blocks', async () => {
    ...
    const rendered = render(
      <VisualConfigEditor spec={spec} schema={selectSchema} />
    )

    expect(await screen.findByText('默认模型')).toBeInTheDocument()
    expect(
      rendered.container.querySelector('.ant-collapse')
    ).toBeInTheDocument()
    expect(
      rendered.container.querySelector('.ant-segmented')
    ).toBeInTheDocument()
    ...
    expect(
      rendered.container.querySelector('.visual-config-inline-textarea')
    ).toBeInTheDocument()
  ...
  it('renders a clearer file module header and visual toolbar', async () => {
    ...
    const moduleMeta = rendered.container.querySelector(
      '.visual-config-module-meta'
    )
  ...
  it('keeps the file header separate from the visual content panels', async () => {
    ...
    const headerPanel = rendered.container.querySelector(
      '.visual-config-header-panel'
    )
    ...
    const contentStack = rendered.container.querySelector(
    ...
    expect(
      contentStack!.querySelector('.visual-config-group')
    ).toBeInTheDocument()
  ...
  it('shows the schema-declared default value next to a field', async () => {
    ...
    expect(
      within(autoUpdatesField.closest('.ant-collapse-header')!).getByText(
        '默认值：true'
      )
    ).toBeInTheDocument()
    ...
    expect(
      within(modelField.closest('.ant-collapse-header')!).queryByText(
        /默认值：/
      )
  ...
  it('opens field details without a nested delayed animation', async () => {
    ...
    expect(detailsRegion.className).not.toContain(
  ...
  it('edits complex object fields with an inline large textarea', async () => {
    ...
    const inlineEditor = screen.getByLabelText(
      '环境变量 内容'
  ...
  it('sorts configured fields first and hides uncommon unset fields by default', async () => {
    ...
    expect(
      configuredField.compareDocumentPosition(unsetField) &
  ...
  it('switches visual field sort order from clickable sort buttons', async () => {
    ...
    expect(
      unsetField.compareDocumentPosition(configuredField) &
        Node.DOCUMENT_POSITION_FOLLOWING
    ).toBeTruthy()
    expect(
      screen.queryByRole('radio', { name: '默认顺序' })
    ).not.toBeInTheDocument()
  ...
  it('saves one visual field without committing other unsaved fields', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('button', { name: '保存默认模型' })
      ).toBeDisabled()
    })
    expect(
      screen.getByRole('button', { name: '保存自动更新' })
    ).not.toBeDisabled()
  })
  ...
  it('does not clip model select popovers inside expanded field details', async () => {
    ...
    const collapseBody = detailsRegion.closest('.ant-collapse-body')
  ...
  it('warns when the selected Claude output style is missing and can create it', async () => {
    ...
    invokeMock.mockImplementation(
      async (command: string, args?: { name?: string }) => {
        if (command === 'read_config_file') {
          return {
            id: 'claude-settings',
            title: 'settings.json',
    ...
    expect(
      screen.getByText('/Users/test/.claude/output-styles/毒舌.md')
    ).toBeInTheDocument()
  ...
  it('persists manually hidden fields and shows them only after expanding more config', async () => {
    ...
    const renderedEditor = render(
      <VisualConfigEditor spec={spec} schema={schema} />
    )
    ...
    expect(
      screen.getByRole('button', { name: '取消隐藏默认模型' })
    ).toBeInTheDocument()
  })
