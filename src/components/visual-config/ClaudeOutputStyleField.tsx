...
function buildStyleSelectOptions(
  styles: ClaudeOutputStyleInfo[],
  missingName: string
) {
  // missingOptions 存储当前配置引用但扫描不到的风格选项。
  const missingOptions = missingName
    ? [{ value: missingName, label: `${missingName}（未找到）` }]
    : []
...
export default function ClaudeOutputStyleField({
  ...
  const styleOptions = useMemo(() => result?.styles ?? [], [result?.styles])
  ...
  const missingSelectedStyle =
    selectedName !== '' && result !== null && !knownStyle && !loading
  // selectedStylePath 存储当前自定义风格应创建或已存在的 Markdown 文件路径。
  const selectedStylePath =
    result && selectedName !== ''
      ? buildStyleFilePath(result.directory, selectedName)
      : ''
      ...
      {!loading && !error && knownStyle && (
        <Card size="small">
          <div className="text-sm font-medium text-text-main">
            {knownStyle.kind === 'custom'
              ? '已找到自定义风格'
              : '已找到内置风格'}
          </div>
          <div className="mt-1 text-xs text-text-muted">
            {knownStyle.description}
          </div>
          {knownStyle.path && (
