  ...
  groups: [
    ...
    },
    {
      ...
      fields: [
        ...
        field("shell_environment_policy", "Shell 环境策略", "控制命令执行时继承、过滤或注入哪些环境变量。", "toml-object", LOCAL_MACHINE_SCOPE, "sensitive", { sensitive: true }),
        field("allow_login_shell", "允许登录 Shell", "允许 Codex 用登录 shell 语义解析环境。", "switch", USER_ONLY_SCOPE, "sensitive"),
    ...
    },
    {
      ...
      fields: [
        ...
        field("project_doc_fallback_filenames", "项目文档兜底文件名", "AGENTS.md 等项目说明文件不存在时尝试读取的兜底文件名。", "string-list", USER_PROJECT_SCOPE),
        field("project_doc_max_bytes", "项目文档最大字节数", "读取项目说明文件时允许的最大字节数。", "number", USER_PROJECT_SCOPE),
        field("project_root_markers", "项目根标记", "用于识别项目根目录的文件或目录名称。", "string-list", USER_PROJECT_SCOPE),
        field("history", "历史记录", "会话历史记录保存、加载和裁剪行为。", "toml-object", USER_ONLY_SCOPE),
    ...
    },
    {
      ...
      fields: [
        ...
        field("hide_agent_reasoning", "隐藏 Agent Reasoning", "在界面中隐藏 agent reasoning 内容。", "switch", USER_ONLY_SCOPE),
    ...
    },
    {
      ...
      fields: [
        ...
        field("mcp_servers", "MCP Servers", "Codex 可连接的 MCP server 配置。", "toml-object", LOCAL_MACHINE_SCOPE, "sensitive", { sensitive: true }),
        ...
        field("plugins", "插件", "Codex 已安装插件启用状态。", "toml-object", USER_ONLY_SCOPE),
        field("marketplaces", "插件市场", "Codex 插件市场来源和更新时间。", "toml-object", USER_ONLY_SCOPE),
