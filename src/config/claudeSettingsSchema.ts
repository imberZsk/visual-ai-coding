  ...
  groups: [
    {
      ...
      fields: [
        ...
        field(
          "outputStyle",
          "输出风格",
          "选择 Claude Code 已知输出风格，或为当前自定义名称创建 output-styles Markdown 文件。",
          "claude-output-style",
          USER_PROJECT_LOCAL_SCOPE,
          "normal"
        ),
    ...
    },
    {
      ...
      fields: [
        ...
        field("sandbox.enabled", "启用 Bash 沙箱", "在支持平台启用 Bash 沙箱。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
        field("sandbox.failIfUnavailable", "沙箱不可用则退出", "启用沙箱但依赖缺失或平台不支持时，启动即失败。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
        field("sandbox.autoAllowBashIfSandboxed", "沙箱内自动允许 Bash", "命令在沙箱内运行时自动批准 Bash。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: true }),
        field("sandbox.excludedCommands", "排除沙箱命令", "需要在沙箱外运行的命令列表。", "string-list", USER_PROJECT_LOCAL_SCOPE, "danger"),
        field("sandbox.allowUnsandboxedCommands", "允许命令跳出沙箱", "允许 dangerouslyDisableSandbox 参数让命令在沙箱外运行。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: true }),
        ...
        field("sandbox.filesystem.allowManagedReadPathsOnly", "只允许托管读取路径", "只采纳托管设置中的 allowRead 路径。", "switch", MANAGED_SCOPE, "danger", { defaultValue: false }),
        ...
        field("sandbox.network.allowManagedDomainsOnly", "只允许托管域名", "只采纳托管设置中的网络允许域名。", "switch", MANAGED_SCOPE, "danger", { defaultValue: false }),
        field("sandbox.network.allowUnixSockets", "允许 Unix Socket", "沙箱内可访问的 Unix socket 路径。", "string-list", USER_PROJECT_LOCAL_SCOPE, "danger"),
        field("sandbox.network.allowAllUnixSockets", "允许所有 Unix Socket", "允许沙箱访问所有 Unix socket。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
        field("sandbox.network.allowLocalBinding", "允许本地端口绑定", "允许沙箱命令绑定 localhost 端口。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
        ...
        field("sandbox.enableWeakerNestedSandbox", "弱化嵌套沙箱", "在非特权 Docker 环境中启用较弱沙箱。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
        field("sandbox.enableWeakerNetworkIsolation", "弱化网络隔离", "允许访问系统 TLS 信任服务，降低网络隔离强度。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
        field("sandbox.allowAppleEvents", "允许 Apple Events", "允许沙箱命令发送 Apple Events。", "switch", USER_PROJECT_LOCAL_SCOPE, "danger", { defaultValue: false }),
    ...
    },
    {
      ...
      fields: [
        ...
        field("disableWorkflows", "禁用动态 Workflow", "禁用动态 workflows 与 bundled workflow commands。", "switch", USER_PROJECT_LOCAL_SCOPE, "normal", { defaultValue: false }),
        field("workflowKeywordTriggerEnabled", "Workflow 关键词触发", "控制 prompt 中 ultracode 关键词是否触发动态 workflow。", "switch", USER_SCOPE, "experimental", { defaultValue: true }),
        field("maxSkillDescriptionChars", "技能描述上限", "限制每个 skill 的描述和 when_to_use 文本进入上下文的字符数。", "number", USER_SCOPE),
        field("skillListingBudgetFraction", "技能列表上下文预算", "为 skill listing 预留的模型上下文窗口比例。", "number", USER_SCOPE, "normal", { defaultValue: 0.01 }),
        ...
        field("autoMemoryEnabled", "自动记忆", "控制 Claude 是否读取和写入自动记忆目录。", "switch", USER_SCOPE, "experimental", { defaultValue: true }),
        ...
        field("respectGitignore", "@ 文件遵守 .gitignore", "控制 @ 文件选择器是否排除 .gitignore 匹配文件。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
    ...
    },
    {
      ...
      fields: [
        field("verbose", "详细输出", "默认显示完整工具输出而非截断摘要。", "switch", USER_PROJECT_LOCAL_SCOPE, "normal", { defaultValue: false }),
        field(
          "theme",
          "主题",
          "Claude Code 界面主题。",
        ...
        field("autoCompactEnabled", "自动压缩上下文", "上下文接近限制时自动 compact 对话。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("cleanupPeriodDays", "清理天数", "启动时删除超过该天数的 session 文件。", "number", USER_SCOPE, "normal", { defaultValue: 30 }),
        field("feedbackSurveyRate", "反馈问卷概率", "符合条件时展示会话质量问卷的概率，0 表示完全关闭。", "number", USER_SCOPE),
        field("fileCheckpointingEnabled", "文件检查点", "每次编辑前快照文件，便于 /rewind 恢复。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("autoScrollEnabled", "自动滚动", "fullscreen 渲染中跟随新输出滚动到底部。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("wheelScrollAccelerationEnabled", "滚轮加速", "快速滚动时加速鼠标滚轮滚动速度。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("terminalProgressBarEnabled", "终端进度条", "在支持的终端中显示进度条。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("awaySummaryEnabled", "离开后摘要", "离开终端数分钟后返回时显示一行 session recap。", "switch", USER_SCOPE),
        field("showTurnDuration", "显示回合耗时", "响应后显示本回合耗时。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("showThinkingSummaries", "显示思考摘要", "在交互会话中显示 extended thinking summaries。", "switch", USER_SCOPE, "normal", { defaultValue: false }),
        field("syntaxHighlightingDisabled", "禁用语法高亮", "禁用 diff、代码块和文件预览的语法高亮。", "switch", USER_SCOPE),
        field("spinnerTipsEnabled", "Spinner Tips", "Claude 工作时在 spinner 中显示 tips。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        ...
        field("agentPushNotifEnabled", "Agent 推送通知", "Remote Control 连接时允许 Claude 主动发送任务完成等手机推送。", "switch", USER_SCOPE, "experimental", { defaultValue: false }),
        field("inputNeededNotifEnabled", "需要输入通知", "Remote Control 连接时，权限提示或问题等待输入时发送推送。", "switch", USER_SCOPE, "experimental", { defaultValue: false }),
    ...
    },
    {
      ...
      fields: [
        ...
        field("includeGitInstructions", "包含 Git 指令", "把内置 commit/PR workflow 指令和 git status 快照加入系统提示。", "switch", USER_PROJECT_LOCAL_SCOPE, "normal", { defaultValue: true }),
        ...
        field("plansDirectory", "计划文件目录", "自定义计划文件存储目录。", "text", USER_SCOPE, "sensitive", { defaultValue: "~/.claude/plans" }),
    ...
    },
    {
      ...
      fields: [
        ...
        field("autoConnectIde", "自动连接 IDE", "从外部终端启动时自动连接运行中的 IDE。", "switch", USER_SCOPE, "normal", { defaultValue: false }),
        field("autoInstallIdeExtension", "自动安装 IDE 扩展", "从 VS Code 终端运行时自动安装 Claude Code IDE 扩展。", "switch", USER_SCOPE, "normal", { defaultValue: true }),
        field("externalEditorContext", "外部编辑器上下文", "用 Ctrl+G 打开外部编辑器时，把 Claude 上次响应作为注释上下文。", "switch", USER_SCOPE, "normal", { defaultValue: false }),
      ],
    },
  ],
};
